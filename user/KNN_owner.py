...
if __name__ == '__main__':
    ...
    d=len(fea_all[0])
    d_stre=2*d
    A,A_inv,B,B_inv,S,W=gen_key(d_stre,d)
    SS=np.array(S)

    last_one_position = S.rfind('1')
    last_zero_position = S.rfind('0')
    WT=generate_random_array(d_stre-d,1,9)
    encrypt_arrp=np.zeros((len(arrp),d_stre))
    pa_en = np.zeros((len(arrp), d_stre))
    pb_en = np.zeros((len(arrp), d_stre))
    p_en = np.zeros((len(arrp), d_stre))
    j=0
    starttime=time.time()#记录特征加密时间
    for i in range(len(arrp)):
        pa_en[j],pb_en[j],p_en[j]=encrypt_p(arrp[i],S,W,A,B,d_stre,WT)
        j=j+1
    print("-----------特征向量加密结束--------------")
